-- ============================================================
-- BiketaBai 3.0 - Optimized & Compacted Database Schema
-- ============================================================
-- This schema is normalized and compacted for better performance
-- Key optimizations:
-- 1. Converted lookup tables to ENUM/VARCHAR fields (removed 4 tables)
-- 2. Removed redundant store fields from users table
-- 3. Optimized indexes
-- 4. Added data cleanup strategies
-- ============================================================
...
CREATE TABLE `users` (
    ...
    `verification_status` varchar(50) CHARACTER SET utf8mb4 NOT NULL DEFAULT 'Pending', -- Pending, Approved, Rejected
...
CREATE TABLE `bikes` (
    ...
    `availability_status` varchar(50) CHARACTER SET utf8mb4 NOT NULL DEFAULT 'Available', -- Available, Rented, Maintenance, Inactive
...
CREATE TABLE `bookings` (
    ...
    `booking_status` varchar(50) CHARACTER SET utf8mb4 NOT NULL DEFAULT 'Pending', -- Pending, Active, Completed, Cancelled
...
CREATE TABLE `payments` (
    ...
    `payment_method` varchar(50) CHARACTER SET utf8mb4 NOT NULL, -- Wallet, GCash, QRPH, Cash, PayMaya, Card
    `amount` decimal(10,2) NOT NULL,
    `payment_status` varchar(50) CHARACTER SET utf8mb4 NOT NULL DEFAULT 'Pending', -- Pending, Completed, Failed, Refunded
...
CREATE TABLE `credit_transactions` (
    ...
    `transaction_type` varchar(50) CHARACTER SET utf8mb4 NOT NULL, -- Load, Withdrawal, Rental Payment, Rental Earnings, Refund, Service Fee
    `amount` decimal(10,2) NOT NULL,
...
CREATE TABLE `ratings` (
    ...
    `booking_id` int NOT NULL,
    `bike_id` int NOT NULL, -- Kept for performance (direct queries without JOIN)
...
CREATE TABLE `reports` (
    ...
    `booking_id` int NULL,
    `status` varchar(50) CHARACTER SET utf8mb4 NOT NULL DEFAULT 'Pending', -- Pending, Assigned, In Progress, Resolved, Closed
    `assigned_to` int NULL,
    `priority` varchar(50) CHARACTER SET utf8mb4 NOT NULL DEFAULT 'Medium', -- Low, Medium, High, Critical
...
-- ============================================================
-- Data Cleanup Strategies (Recommendations)
-- ============================================================

-- 1. Archive old location tracking data (older than 30 days)
-- DELETE FROM location_tracking WHERE tracked_at < DATE_SUB(NOW(), INTERVAL 30 DAY);

-- 2. Clean up expired/verified OTPs (older than 24 hours)
-- DELETE FROM phone_otps WHERE (is_verified = 1 AND verified_at < DATE_SUB(NOW(), INTERVAL 24 HOUR)) OR expires_at < NOW();

-- 3. Archive old read notifications (older than 90 days)
-- DELETE FROM notifications WHERE is_read = 1 AND created_at < DATE_SUB(NOW(), INTERVAL 90 DAY);

-- Note: Implement these cleanup jobs as scheduled tasks or background services
-- in your application rather than running them directly in SQL.

-- ============================================================
-- Schema Statistics
-- ============================================================
-- Total Tables: 16 (reduced from 20)
-- Lookup Tables Removed: 4
--   - availability_statuses (converted to VARCHAR)
--   - booking_statuses (converted to VARCHAR)
--   - payment_methods (converted to VARCHAR)
--   - transaction_types (converted to VARCHAR)
-- Fields Removed from users: 5 (store fields)
-- Total Foreign Keys: 18 (reduced from 22)
-- ============================================================
